...
export const ToolTip: FunctionComponent<ToolTipProps> = ({ toolTipText }) => {
  ...
  return (
    <div className="relative inline-block">
      <div onClick={() => setDisplayInfo((prevState) => !prevState)}>
        <HelpCircle className="h-5 w-5 cursor-pointer" />
      </div>
      {displayInfo && (
        <>
          <div className="absolute z-20 top-8 left-12 bg-white p-4 shadow-md w-72 max-w-xs text-sm border">
            {toolTipText}
          </div>
          <div className="fixed top-0 left-0 z-10 w-screen h-screen" onClick={() => setDisplayInfo(false)} />
        </>
      )}
    </div>
  );
